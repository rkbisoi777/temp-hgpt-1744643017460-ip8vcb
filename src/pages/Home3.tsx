...
export function Home() {
    ...
    useEffect(() => {
        const fetchProperties = async () => {
            try {
                const props: Property[] = await propertyService.searchLocationProperties(location);
                setProperties(props);
            } catch (error) {
                console.error("Error fetching properties:", error);
    ...
    const handleSearch = async (query: string) => {
        if (!user) {
            ...
            try {
                ...
                // Navigate to the new chat with the query
                navigate(`/chat/${newChat.id}`, { state: { initialQuery: query } });
            } catch (error: any) {
